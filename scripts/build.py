...
def build_lang(lang_code, dist_root):
    ...
    # 2. Reconstruir la HOME (Nodo Raíz '/') con numeración jerárquica y colapsables
    home_html = '<div class="curriculum-container">\n'
    chapter_idx = 1
    for section_name, lessons in structure.items():
        home_html += f'''<section class="curriculum-section" data-section-id="{chapter_idx}">
...
'''
        ...
        for l in lessons:
            ...
            h_num = f"{chapter_idx}.{lesson_idx}"
            l['hierarchical_num'] = h_num
                    ...
                    for section, lessons in structure.items():
                        ...
                        for lesson in lessons:
                            if lesson['id'] == lesson_id:
                                title = lesson['title']
                                h_num = f"{c_count}.{l_count}"
                                chapter = f"{c_count}. {section}"
                                break
                            l_count += 1
                        if chapter: break
                        c_count += 1
    ...
    # Escribir search.json
    with (out_dir / 'search.json').open('w', encoding='utf-8') as f:
        json.dump(search_index, f, ensure_ascii=False)
    
    print(f"[OK] Lessons compiled and search index generated for {lang_code}.")
...
def build_all():
    ...
    src_dir = Path('src')
    static_files = ['index.html', 'app.js', 'sw.js', 'manifest.json', 'style.css']
